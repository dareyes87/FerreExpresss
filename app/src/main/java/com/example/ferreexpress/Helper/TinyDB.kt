...
class TinyDB(private val appContext: Context) {
    ...
    fun getListInt(key: String?): ArrayList<Int> {
        val myList: Array<String> = TextUtils.split(preferences.getString(key, ""), "‚‗‚")
    ...
    fun <T> getObject(key: String?, classOfT: Class<T>?): T {
        ...
        if (value == null) {
            throw NullPointerException("Value was null for key: $key")
        }
        return value
    }
    ...
    fun putListInt(key: String?, intList: ArrayList<Int>?) {
        checkForNullKey(key)
        val myIntList = intList?.toTypedArray() ?: arrayOf()
    ...
    fun putListLong(key: String?, longList: ArrayList<Long>?) {
        checkForNullKey(key)
        val myLongList = longList?.toTypedArray() ?: arrayOf()
        preferences.edit().putString(key, TextUtils.join(",_,", myLongList)).apply()
    }
    ...
    fun putListDouble(key: String?, doubleList: ArrayList<Double>?) {
        checkForNullKey(key)
        val myDoubleList = doubleList?.toTypedArray() ?: arrayOf()
    ...
    fun putListString(key: String?, stringList: ArrayList<String>?) {
        checkForNullKey(key)
        val myStringList = stringList?.toTypedArray() ?: arrayOf()
